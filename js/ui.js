...
export function renderClassList(students, columns, { isEditing = false, merges = [], hierarchies = {}, selectedCells = [] } = {}) {
    if (!columns || columns.length === 0) return '';

    const isCellMerged = (rIdx, colKey) => {
        const merge = merges.find(m => rIdx >= m.r1 && rIdx <= m.r2 && colKey === m.colKey);
        if (merge && rIdx > m.r1) return { hidden: true };
        if (merge && rIdx === m.r1) return { rowSpan: m.r2 - m.r1 + 1 };
        return { hidden: false };
    };

    const isSelected = (r, k) => selectedCells.some(c => c.r === r && c.k === k);

    // Render 2 lớp tiêu đề nếu có phân tầng
    let hierarchyRow = '';
    const hasHierarchy = Object.keys(hierarchies).length > 0;
    if (hasHierarchy) {
        hierarchyRow = `<tr class="bg-indigo-100 dark:bg-indigo-900/40 text-indigo-900 dark:text-indigo-100 font-bold text-[10px] uppercase">`;
        columns.forEach(col => {
            const h = hierarchies[col.key];
            hierarchyRow += `<th class="border border-gray-200 dark:border-gray-700 p-1 text-center">${h || ''}</th>`;
        });
        if (isEditing) hierarchyRow += `<th class="border border-gray-200 dark:border-gray-700 p-1"></th>`;
        hierarchyRow += `</tr>`;
    }
    ...
    const headerHtml = `
        ...
        <tr class="bg-indigo-50 dark:bg-indigo-900/20 text-indigo-900 dark:text-indigo-100 font-bold uppercase text-xs tracking-wider">
            ${columns.map(h => `<th class="border border-gray-200 dark:border-gray-700 p-3 text-center cursor-context-menu" data-col-key="${h.key}">${h.label}</th>`).join('')}
    ...
    const bodyHtml = students.map((s, rIdx) => {
        const cells = columns.map(h => {
            const mergeInfo = isCellMerged(rIdx, h.key);
            if (mergeInfo.hidden) return '';
            const selectedClass = isSelected(rIdx, h.key) ? 'bg-indigo-100 ring-2 ring-indigo-500 ring-inset' : '';
            return `
                <td class="border border-gray-200 dark:border-gray-700 p-0 text-sm text-center ${selectedClass}" 
                    rowspan="${mergeInfo.rowSpan || 1}" 
                    data-row-idx="${rIdx}" data-col-key="${h.key}">
                    ${isEditing ? 
                        `<textarea class="w-full bg-white dark:bg-transparent outline-none text-center p-2 resize-none block border-none focus:bg-indigo-50" 
                                   data-action="edit-cell" data-row-idx="${rIdx}" data-col-key="${h.key}">${s[h.key] || ''}</textarea>` : 
                        `<div class="p-2 whitespace-pre-wrap">${s[h.key] || ''}</div>`
                    }
                </td>
            `;
        }).join('');
        return `<tr class="bg-white dark:bg-gray-900">${cells}${isEditing ? `<td class="border border-gray-200 dark:border-gray-700 p-2 text-center"><button data-action="delete-student" data-row-idx="${rIdx}" class="text-red-500 hover:scale-110 transition-transform">${icons.trash}</button></td>` : ''}</tr>`;
    }).join('');
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full border-collapse">
                    ...
                    <tbody>${bodyHtml || `<tr><td colspan="${columns.length + 1}" class="p-8 text-center text-gray-400">Trống</td></tr>`}</tbody>
                </table>
            </div>
            ${isEditing ? `<div class="p-3 bg-gray-50 border-t flex justify-center"><button data-action="add-student" class="flex items-center gap-2 px-6 py-2 bg-indigo-600 text-white rounded-lg font-bold hover:bg-indigo-700 shadow-md transition-all">${icons.plus} Thêm dòng mới</button></div>` : ''}
        </div>
    `;
}
                                ...
                                <tr>
                                    ...
                                    ${days.map(d => {
                                        ...
                                        return `<td class="border dark:border-gray-700 p-1">
                                            ${isEditing ? `<textarea data-day="${d}" data-session="${s}" data-index="${i}" class="w-full h-12 p-1 bg-white border-none text-center focus:bg-indigo-50 rounded resize-none text-sm transition-colors">${val}</textarea>` : `<div class="min-h-[3rem] flex items-center justify-center p-2">${val}</div>`}
                                        </td>`;
                                    }).join('')}
                                </tr>
                            `).join('')}
